...
function theme_commerce_cart_line_item(variables) {
  try {
    ...
    '<p><strong>Цена</strong>: ' + variables.line_item.commerce_unit_price_formatted + '</p>';
    ...
    html += '<p class="ui-li-aside"><strong>Всего</strong>: ' +
...
function theme_commerce_cart_line_item_quantity(variables) {
  try {
    ...
    var html = '<label for="' + id + '">Количество</label>' + input + '';
    return html;
  }
...
function theme_commerce_cart_total(variables) {
  try {
    return '<h3 class="ui-bar ui-bar-a ui-corner-all">Итого: ' +
